...
const STATE_SSO_PROVIDERS = {};
// Example: STATE_SSO_PROVIDERS['maharashtra'] = { clientId, clientSecret, ... }

const SUPPORTED_SSO_PROVIDERS = ['google', ...Object.keys(STATE_SSO_PROVIDERS).map(state => `state_${state}`)];
 ...
 */
function extractExternalIdentity(provider, tokenPayload) {
  if (provider === 'google') {
    return {
      provider: 'google',
      idtype: 'sub',
 ...
 */
function extractUserInfoFromToken(tokenPayload) {
  return {
...
// ──────────────────────────────────────────────────────────────────────────────
app.get('/iam/sso/:provider/login', async (req, res) => {
  try {
...
// ──────────────────────────────────────────────────────────────────────────────
app.get('/iam/sso/:provider/callback', async (req, res) => {
  try {
    const { provider } = req.params;
    ...
    try {
      ...
      if (resolveErr.response?.status === 409) {
        // Conflict - return to frontend for manual resolution
        await stateStore.delete(state);
